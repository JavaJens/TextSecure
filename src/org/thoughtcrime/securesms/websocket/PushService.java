...
import org.whispersystems.websocket.messages.protobuf.SubProtocol.WebSocketMessage;
import org.whispersystems.websocket.messages.protobuf.SubProtocol.WebSocketRequestMessage;
import org.whispersystems.websocket.messages.protobuf.SubProtocol.WebSocketResponseMessage;
...
public class PushService extends Service implements Listener {
  ...
  private static final int HEARTBEAT = 15;
  ...
  public int onStartCommand(Intent intent, int flags, int startId) {
    ...
    if (intent == null || ACTION_CONNECT == intent.getAction() || Util.isEmpty(intent.getAction())) {
      ...
      if (operation == null) {
        ...
        am.setInexactRepeating(AlarmManager.RTC_WAKEUP, System.currentTimeMillis(), HEARTBEAT * MILLIS, operation);
      }
    }
  ...
  public synchronized void onMessage(byte[] data) {
    ...
    try {
      ...
      WebSocketMessage websocketMessage = WebSocketMessage.parseFrom(data);
  ...
  private void processWebSocketResponseMessage(WebSocketRequestMessage request) {
    Log.w(TAG, "Client should not receive WebSocket Respond messages");
  }

  private void processWebSocketRequestMessage(WebSocketRequestMessage request) {
  ...
  private void sendWebSocketMessageAcknowledgement(WebSocketRequestMessage request) {
    WebSocketResponseMessage response = WebSocketResponseMessage.newBuilder().
                    setId(request.getId()).setStatus(200).setMessage("OK")
            .build();
